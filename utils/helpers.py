...
from psutil import Process, REALTIME_PRIORITY_CLASS
...
def clean_temp_files():
    ...
    try:
        data_removed: int = 0
        for file_f in track(temp_files,
        ...
        for file_f in track(windows_temp_files,
        ...
        clear_recyclebin = str(console.input(
            f"[yellow]#[/yellow]> [bold cyan]Recycle Bin[/bold cyan] [deep_pink4][{len(list(winshell.recycle_bin()))}][/deep_pink4] [bold cyan]Items Found[/bold cyan] | [red]Roughly [{convert_bytes(get_folder_size_bytes("C:\\$RECYCLE.BIN"))}] Found[/red]\n[yellow]#[/yellow]> [white]Clear[/white] [bold cyan]Recycle Bin?[/bold cyan][bold white](Y/N)[/bold white]\n[yellow]$[/yellow]> "))
...
def scan_dir(path_argument: str):
    ...
    if not path.exists(data_path):
        func_console.print(
            f"[bold red]Error[/bold red]: [red]Unable to find [bold green]{data_path}[/bold green][/red]")
            ...
            with ThreadPoolExecutor(max_workers=cpu_count()) as executor:  # Adjust max_workers as needed
                ...
                for child in root_:
                    if child.is_dir():
                        ...
                        json_data_path_size[driver].append(
                            {"path": child.path.encode('ascii', 'replace').decode(), "size": file_size})
    ...
    def load_driver_(driver_):
        ...
        if path_argument == "" or path_argument == "." or (
                path_argument.split(":")[0] in availible_drives() and path_argument.split(":")[1] == "/"):
...
def scan_all_msgfiles():
    ...
    try:
        ...
        _console_.print(
            f"[yellow]#[/yellow]> [red]No Data Found For Drives[Ensure Drives Folder Exists with Drive Data/s]. OR Developer Error?[/red]")
    ...
    try:
        ...
        _console_.print(
            f"[yellow]#[/yellow]> [red]No Data Found In CustomPaths Saved[Ensure CustomPaths Folder Exists with Data Files From Past Scanned and Saved Data]. OR Developer Error?[/red]")
